...
export function AuthForm({ mode }: { mode: "sign-in" | "sign-up" }) {
  ...
  async function handleOAuth(provider: "oauth_google" | "oauth_github") {
    const label = provider === "oauth_google" ? "google" : "github";
    setLoading(label);
    setError(null);
    try {
      if (isSignIn) {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      if (isSignIn) {
        ...
        if (passError) {
          setError(passError.message ?? "Invalid email or password.");
          setLoading(null);
          return;
        }
        if (signIn!.status === "complete") {
          const { error: finalizeError } = await signIn!.finalize();
          if (!finalizeError) router.push("/dashboard");
          else setError(finalizeError.message ?? "Error finalizing sign in.");
        }
      } else {
        const { error: passError } = await signUp!.password({
          emailAddress: email,
          password,
        });
        if (passError) {
          setError(passError.message ?? "Could not create account.");
          setLoading(null);
          return;
        }
        if (
          ...
          await signUp!.verifications.verifyEmailCode({ code: "email_code" });
        ...
        if (signUp!.status === "complete") {
          ...
          if (!finalizeError) router.push("/dashboard");
          else setError(finalizeError.message ?? "Error finalizing sign up.");
        }
      }
    } catch (err: any) {
